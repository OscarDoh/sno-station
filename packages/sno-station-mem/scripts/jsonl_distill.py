...
write a compact batch file for a distiller agent to turn into edge-claw memories.
...
This script does NOT call any LLM or write to the database. It only prepares
data for the distiller agent.
"""
...
def _read_jsonl_lines(
    path: Path, start_offset: int, max_bytes: int
) -> Tuple[List[Tuple[str, int]], int]:
    """Read up to max_bytes from path. Returns (line, next_offset) pairs and end_offset."""
...
def _clean_text(s: str) -> str:
    ...
    s = re.sub(
        r"^Replied message \(untrusted, for context\):\s*\n+",
        "",
        s,
        flags=re.IGNORECASE,
    )
...
def _is_noise(s: str) -> bool:
    ...
    if "[queued messages while agent was busy]" in lower:
        return True
    if (
        "you are running a boot check" in lower
        or "boot.md — gateway startup health check" in lower
    ):
        return True
...
def _acquire_cursor_lock(handle: Any) -> None:
    ...
    if msvcrt is not None:
        ...
        if locking is None or lock_mode is None:
...
def _release_cursor_lock(handle: Any) -> None:
    if fcntl is not None:
    ...
    if msvcrt is not None:
        ...
        if locking is None or unlock_mode is None:
            raise RuntimeError("Windows file-locking backend is unavailable")
        handle.seek(0)
...
class _CursorLock:
    ...
    def __exit__(self, exc_type, exc, tb) -> None:
        if self._fh is None:
            return
        try:
            _release_cursor_lock(self._fh)
        finally:
            self._fh.close()


def _cursor_lock_path(state_dir: Path) -> Path:
    return state_dir / "cursor.lock"
...
def init_from_now(state_dir: Path, agents_dir: Path) -> Dict[str, Any]:
    ...
    with _CursorLock(_cursor_lock_path(state_dir)):
    ...
    with _CursorLock(_cursor_lock_path(state_dir)):
        ...
        for agent_id, f in _list_session_files(agents_dir):
            ...
            if entry and entry.get("inode") == inode:
                ...
                # The init command owns "start from now"; run must capture first-seen live files.
                committed = 0
            ...
            consumed_offset = committed
            for line, line_end_offset in lines:
                try:
                    obj = json.loads(line)
                except Exception:
                    consumed_offset = line_end_offset
                    continue
                ...
                current_count = agent_message_counts.get(agent_id, 0)
                if (
                    max_messages_per_agent > 0
                    and current_count >= max_messages_per_agent
                ):
                    break
                ...
                agent_message_counts[agent_id] = current_count + 1
                consumed_offset = line_end_offset

            if not extracted:
                if consumed_offset > committed:
                    # Advance past malformed, non-message, or noise lines only.
                    files[key] = {
                        "agentId": agent_id,
                        "inode": inode,
                        "committed": consumed_offset,
                        "pending": None,
                        "pendingBatch": None,
                        "lastSize": size,
                        "updatedAtMs": _now_ms(),
        ...
        batch_id = f"{time.strftime('%Y%m%d-%H%M%S')}-{time.time_ns()}-{os.getpid()}"
        ...
        batch_path.write_text(
            json.dumps(batch_obj, ensure_ascii=False, indent=2) + "\n", "utf-8"
        )
...
def main() -> int:
    ...
    sub.add_parser(
        "init", help="Initialize cursor to EOF for all current session files"
    )

    s_run = sub.add_parser(
        "run", help="Extract incremental message tail and create a batch file"
    )
    ...
    s_commit = sub.add_parser(
        "commit", help="Commit a processed batch (advance committed offsets)"
    )
